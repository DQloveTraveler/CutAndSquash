...
namespace naichilab
{
    public class RankingSceneManager : MonoBehaviour
    {
        ...
        IEnumerator GetHighScoreAndRankingBoard()
        {
            ...
            captionLabel.text = string.Format("{0}\nランキング", _board.BoardName);
            ...
            else
            {
                ...
                {
                    //数値が低い方が高スコア
                    IsHighScore = _lastScore.Value < highScore.Value;
                }
                else
                {
                    //数値が高い方が高スコア
                    IsHighScore = highScore.Value < _lastScore.Value;
                }
                sendScoreButton.interactable = IsHighScore;
        ...
        private IEnumerator LoadRankingBoard()
        {
            ...
            else if (so.Count > 0)
            {
                ...
                foreach (var r in so.Result)
                {
                    ...
                    {
                        case 1:
                            rankNode.NoText.fontMaterial = fontAssets[0].material;
                            break;
                        case 2:
                            rankNode.NoText.fontMaterial = fontAssets[1].material;
                            break;
                        case 3:
                            rankNode.NoText.fontMaterial = fontAssets[2].material;
                            break;
                        default:
                            rankNode.NoText.fontMaterial = fontAssets[3].material;
                            break;
                    }
